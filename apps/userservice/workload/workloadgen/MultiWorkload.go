...
func (w *multiWldGen) Run(ctx context.Context) error {
	...
	w1.AddAPI("NA", w.RunNAHandler, 75)
	w1.AddAPI("EU", w.RunEUHandler, 5)
	w1.AddAPI("SA", w.RunSAHandler, 5)
	w1.AddAPI("AS", w.RunASHandler, 5)
	w1.AddAPI("AF", w.RunAFHandler, 5)
	w1.AddAPI("OC", w.RunOCHandler, 5)
	w2 := workload.NewWorkload()
	w2.AddAPI("NA", w.RunNAHandler, 5)
	w2.AddAPI("EU", w.RunEUHandler, 5)
	w2.AddAPI("SA", w.RunSAHandler, 5)
	w2.AddAPI("AS", w.RunASHandler, 5)
	w2.AddAPI("AF", w.RunAFHandler, 5)
	w2.AddAPI("OC", w.RunOCHandler, 75)
