...
func AddAverageLatencyMeter(spec wiring.WiringSpec, serviceName string) {
	serverWrapper := serviceName + ".server.avglatmeter"
	...
	spec.Define(serverWrapper, &AverageLatencyServerWrapper{}, func(ns wiring.Namespace) (ir.IRNode, error) {
		var server golang.Service

		if err := ns.Get(serverNext, &server); err != nil {
			return nil, blueprint.Errorf("Average Latency Meter %s expected %s to be a golang.Service, but encountered %s", serverWrapper, serverNext, err)
		}

		return newAverageLatencyServerWrapper(serverWrapper, server)
	})
}
